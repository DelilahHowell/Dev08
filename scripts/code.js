...
#################################################*/
  ...
  $newItemForm.on("submit", function(e) {
    ...
    e.preventDefault(); // Prevent form being submitted
		...
		var favIcon = `class = "favorite"`;//**added favIcon to new entries**
		var isDup = dupchk(text);
		if (isDup === false){
			$list.append("<li "+ favIcon + ">" + text + "</li>")
		} // Add item to end of the list
		else if(isDup === true){
			window.alert(text + " Duplicate not valid");
		}
    $("input:text"
	).val(""); // Empty the text input
	//****updateCount missing "()"****
